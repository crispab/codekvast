...
public class SampleApp {
  ...
  private void measureMethodCallTrackingOverhead() {
    ...
    int overheadNanosUntrackedLoggedComparedToUntrackedUnlogged =
        (int)
            ((untrackedLoggedElapsedMillis - untrackedNotLoggedElapsedMillis)
                * 1000000d
                / (double) count);
    int overheadNanosTrackedComparedToUntracked =
        (int)
            ((trackedElapsedMillis - untrackedNotLoggedElapsedMillis) * 1000000d / (double) count);
    int overheadNanosTrackedComparedToLoggerCall =
        (int)
            ((trackedElapsedMillis - untrackedLoggedElapsedMillis - untrackedNotLoggedElapsedMillis)
                * 1000000d
                / (double) count);

    System.out.println();
    System.out.printf(
        "Invoked a trivial untracked          method %,d times in %5d ms%n",
        count, untrackedNotLoggedElapsedMillis);
    System.out.printf(
        "Invoked a trivial untracked   logged method %,d times in %5d ms%n",
        count, untrackedLoggedElapsedMillis);
    System.out.printf(
        "Invoked a trivial   tracked unlogged method %,d times in %5d ms%n",
        count, trackedElapsedMillis);
    System.out.println();
    System.out.printf(
        "Calling a disabled logger adds roughly %d ns compared to a plain method call%n",
        overheadNanosUntrackedLoggedComparedToUntrackedUnlogged);
    System.out.printf(
        "Codekvast                 adds roughly %d ns compared to a plain method call%n",
        overheadNanosTrackedComparedToUntracked);
    System.out.printf(
        "Codekvast                 adds roughly %d ns compared to a logger call%n",
        overheadNanosTrackedComparedToLoggerCall);
    System.out.println();
    ...
    if (overheadNanosTrackedComparedToUntracked <= 5) {
      throw new IllegalStateException(
          "Unreasonable overhead: "
              + overheadNanosTrackedComparedToUntracked
              + ". Is Codekvast Agent correctly wired?");
    }
  }
  ...
  private void tryToLoadClass(String className, boolean expectedSuccess) {
    try {
      ...
      System.out.printf(
          "%s: %s cannot load class %s%n", result, SampleApp.class.getName(), className);
    }
  }
