...
. Download https://bintray.com/artifact/download/crisp/foobar/agent/foobar-agent-{codekvast-version}
.zip[codekvast-agent-{codekvast-version}.zip] to any directory
...
----
sudo -s
unzip path/to/codekvast-agent-{codekvast-version}.zip -d /opt <1>
cd /opt
ln -s codekvast-agent-{codekvast-version} codekvast-agent
cd /opt/codekvast-agent/conf
cp codekvast-agent.properties.sample.{codekvast-version} codekvast-agent.properties # <2>
cd /etc/init.d
cp /opt/codekvast-agent/etc/init.d/codekvast-agent .
chkconfig --add codekvast-agent # <3>
----
<1> Substitute `path/to` with the actual path to the downloaded zip file.
<2> Edit codekvast-agent.properties to suit your needs.
<3> This works for Redhat-based Linux distros. For Debian-based distros, use `update-rc.d codekvast-agent defaults` instead.

Once you have finished editing codekvast-agent.properties, execute

----
/etc/init.d/codekvast-agent start
----
...
----
<1> Ssubstitute `path\to` with the actual path were Codekvast is installed.
<2> Substitute `path\to` with the actual path were Tomcat is installed.
<3> Edit `bin\setenv.sh` so that CODEKVAST_HOME matches the path were codekvast-agent-{codekvast-version} is installed.
...
Download https://bintray.com/artifact/download/crisp/foobar/agent/foobar-server-{codekvast-version}.zip[codekvast-server.zip] to any
directory

[source,bash,subs="attributes,verbatim"]
----
sudo -s
unzip path/to/codekvast-server-{codekvast-version}.zip -d /opt # <1>
cd /opt
ln -s codekvast-server-{codekvast-version} codekvast-server
cd /opt/codekvast-server/conf
cp codekvast-server.properties.sample.{codekvast-version} codekvast-server.properties <2>
cd /etc/init.d
cp /opt/codekvast-server/etc/init.d/codekvast-server .
chkconfig --add codekvast-server <3>
----
<1> Substitute `path/to` with the actual path to the downloaded zip file.
<2> Edit codekvast-server.properties to suit your needs.
<3> This works for Redhad-based Linux distros. For Debian-based distros, use `update-rc.d codekvast-server defaults` instead.
...
=== Codekvast Agent
The agent reads it config from /opt/codekvast-agent/conf/codekvast-agent.conf. The sample configuration file in that directory is self
documenting.
