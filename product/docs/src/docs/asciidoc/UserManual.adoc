...
:aspectj-ltw-configuration: https://eclipse.org/aspectj/doc/next/devguide/ltw-configuration.html
...
. Download
https://bintray.com/artifact/download/crisp/foobar/agent/foobar-agent-{codekvast-version}.zip[codekvast-agent-{codekvast-version}.zip]
to any directory
...
ln -s /opt/codekvast-agent/etc/init.d/codekvast-agent /etc/init.d
chkconfig --add /etc/init.d/codekvast-agent # <3>
----
<1> Substitute `path/to` with the actual path to the downloaded zip file.
<2> Edit codekvast-agent.properties to suit your needs. See <<configuring-codekvast-agent, Configuring Codekvast Agent>>.
<3> This works for Redhat-based Linux distros. For Debian-based distros, use `update-rc.d /etc/init.d/codekvast-agent defaults` instead.
...
[TIP]
====
There is a helper script called `/opt/codekvast-agent/bin/showJvmParams.sh` that when invoked produces a valid JVM_OPTS that can be pasted
into your
...
ln -s /opt/codekvast-server/etc/init.d/codekvast-server /etc/init.d
chkconfig --add /etc/init.d/codekvast-server <3>
----
<1> Substitute `path/to` with the actual path to the downloaded zip file.
<2> Edit codekvast-server.properties to suit your needs. See <<configuring-codekvast-server, Configuring Codekvast Server>>.
<3> This works for Redhad-based Linux distros. For Debian-based distros, use `update-rc.d /etc/init.d/codekvast-server defaults` instead.
