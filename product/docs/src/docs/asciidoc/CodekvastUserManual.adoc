...
rm /opt/codekvast-agent
ln -s /opt/codekvast-agent-{codekvast-version} /opt/codekvast-agent
cd /opt/codekvast-agent/conf
cp codekvast-agent.properties.sample.{codekvast-version} codekvast-agent.properties # <1>
rm /etc/init.d/codekvast-agent
ln -s /opt/codekvast-agent/etc/startscript/debian/codekvast-agent /etc/init.d
mkdir -p /var/log/codekvast
update-rc.d codekvast-agent defaults # <2>
----
<1> Edit codekvast-agent.properties to suit your needs. See <<configuring-codekvast-agent, Configuring Codekvast Agent>>.
<2> This works for Debian-based Linux distros. For Redhat-based distros, use `chkconfig --add /etc/init.d/codekvast-agent` instead.
