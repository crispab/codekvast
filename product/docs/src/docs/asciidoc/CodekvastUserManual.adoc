...
sudo ln -s /opt/codekvast-daemon-{codekvast-version}/codekvast-daemon.jar /etc/init.d/codekvast-daemon # <2>
sudo update-rc.d codekvast-daemon defaults # <3>
sudo vi /opt/codekvast-daemon-{codekvast-version}/application.properties # <4>
----
...
<2> This works for  Linux distros that support System-V.
<3> This works for Debian-based distros. For Redhat, use `chkconfig --add` instead.
<4> Configure the daemon to suit your needs.
...
<4> This works for Debian based distros. For Redhat, use `chkconfig --add` instead.
...
|A comma-separated list of strings.
|excludePackagePrefixes = com.acme.timecritical, foo.bar.even.more.time.critical
|
