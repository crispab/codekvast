...
+-------------------------|----------------------------------------------------+         +-----|---------------------+
                          |                                                                    |
                          |<-=------------Manual file transfer/HTTP file upload-------------=->|
                          |                                                                    |
                          \----------------\                                /------------------/
                                           |                                |
                                           v                                v
...
wget {bintrayDownloadPrefix}/codekvast-warehouse-{codekvast-version}.zip
unzip codekvast-warehouse-{codekvast-version}.zip -d /opt
rm /opt/codekvast-warehouse
ln -s /opt/codekvast-warehouse-{codekvast-version} /opt/codekvast-warehouse
cd /opt/codekvast-warehouse/conf
cp codekvast-warehouse.properties.sample.{codekvast-version} codekvast-warehouse.properties # <1>
mkdir -p /var/log/codekvast
rm /etc/init.d/codekvast-warehouse
ln -s /opt/codekvast-warehouse/etc/startscript/debian/codekvast-warehouse /etc/init.d # <2>
update-rc.d codekvast-warehouse defaults # <2>
----
<1> Edit codekvast-warehouse.properties to suit your needs. See <<configuring-codekvast-warehouse, Configuring Codekvast Warehouse>>.
<2> This works for Debian-based Linux distros. For Redhat-based distros, use `chkconfig --add /etc/init.d/codekvast-warehouse` instead.
