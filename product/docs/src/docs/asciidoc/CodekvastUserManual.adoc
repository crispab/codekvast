...
sudo rm -f /etc/init.d/codekvast-daemon
...
to make sure that SCP works without asking for a password:
----
sudo su - $RUN_DAEMON_AS_USER
...
<3> Upload the public key to the target server
<4> Validate that you can login without password
<5> Logout from the remote host
<6> Exit from the RUN_DAEMON_AS_USER shell

Once you have finished editing application.properties (and uploaded the public SSH key), execute

[source,bash,subs="attributes,verbatim"]
----
sudo /etc/init.d/codekvast-daemon start
----
